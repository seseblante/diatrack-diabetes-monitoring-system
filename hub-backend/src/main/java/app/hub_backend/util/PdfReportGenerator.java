...
public class PdfReportGenerator {
    ...
    private static final float SECTION_SIZE = 14f;
    private static final float TEXT_SIZE = 10f;

    private static final BigDecimal SEVERE_LOW = new BigDecimal("54");
    private static final BigDecimal SEVERE_HIGH = new BigDecimal("250");
    ...
    public byte[] generatePatientReport(
        ...
        addIfNotEmpty(sections, "Clinician Notes",
                clinicianNotes, this::formatClinicianNote);

        addIfNotEmpty(sections, "Current Medications",
                regimens, this::formatRegimen);

        addIfNotEmpty(sections, "Medication Logs",
                medicationLogs, this::formatMedicationLog);

        addIfNotEmpty(sections, "Glucose Readings",
                glucoseReadings, this::formatGlucoseReading);

        addIfNotEmpty(sections, "Meal Logs",
                meals, this::formatMeal);

        addIfNotEmpty(sections, "Symptom Notes",
                symptomNotes, this::formatSymptomNote);
    ...
    private byte[] buildPdf(String title, String subtitle, List<Section> sections) {
        try (PDDocument doc = new PDDocument();
            ...
            // Title
            w.centered(title, bold, TITLE_SIZE);
            w.down(8);
            w.centered(subtitle, regular, SUBTITLE_SIZE);
            w.rule();
            w.down(10);
    ...
    private Section createSummarySection(
        ...
        lines.add("Medication Compliance: " + compliance);

        if (readings == null || readings.isEmpty()) {
            lines.add("No glucose data available.");
            return new Section("At-a-Glance Summary", lines);
        }
        ...
        long severeHighs = readings.stream()
                .filter(r -> r.getValueMgdl().compareTo(SEVERE_HIGH) >= 0)
                .count();

        long severeLows = readings.stream()
                .filter(r -> r.getValueMgdl().compareTo(SEVERE_LOW) <= 0)
                .count();

        lines.add(String.format("Average Glucose: %.1f mg/dL", avg));
        lines.add("Severe High Events (>250): " + severeHighs);
        lines.add("Severe Low Events (<54): " + severeLows);
        lines.add("Total Readings: " + readings.size());
        lines.add("Meals Logged: " + (meals != null ? meals.size() : 0));
        lines.add("Medications Logged: " + (meds != null ? meds.size() : 0));
    ...
    private String formatGlucoseReading(GlucoseReading r) {
        return r.getMeasuredAt().format(DateTimeFormatter.ofPattern("MMM d, h:mm a")) +
                " • " + r.getValueMgdl() + " mg/dL (" + r.getContext().getDisplayName() + ")";
    }

    private String formatMeal(Meal m) {
        return m.getLoggedAt().format(DateTimeFormatter.ofPattern("MMM d, h:mm a")) +
                " • " + m.getDescription() +
                (m.getCarbsG() != null ? " (" + m.getCarbsG() + "g carbs)" : "");
    }

    private String formatMedicationLog(MedicationLog log) {
        return log.getTakenAt().format(DateTimeFormatter.ofPattern("MMM d, h:mm a")) +
                " • Took " + log.getRegimen().getMedicationName();
    }

    private String formatClinicianNote(ClinicianNote n) {
        return n.getUpdatedAt().format(DateTimeFormatter.ofPattern("MMM d, yyyy")) +
                " — " + n.getNoteContent();
    }
    ...
    private static class Writer {
        ...
        void centered(String text, PDFont font, float size) throws IOException {
            float width = font.getStringWidth(text) / 1000 * size;
            cs.beginText();
            cs.setFont(font, size);
            cs.newLineAtOffset((PDRectangle.A4.getWidth() - width) / 2, y);
            cs.showText(text);
            cs.endText();
            y -= size + 6;
        }

        void sectionHeader(String text) throws IOException {
            ensure(SECTION_SIZE + 10);
            cs.beginText();
            cs.setFont(PDType1Font.HELVETICA_BOLD, SECTION_SIZE);
            cs.newLineAtOffset(MARGIN, y);
            cs.showText(text);
            cs.endText();
            y -= SECTION_SIZE + 4;
